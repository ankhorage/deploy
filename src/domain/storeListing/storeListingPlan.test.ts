...
const current: StoreListingTargetState = {
  ...
  locales: [{ locale: 'de-CH', name: 'Ankh', summary: 'Kurz' }],
  assetSets: [{
    target: 'android', locale: 'de-CH', variant: 'phone', checksum: 'sha256', hashes: ['sha'],
  }],
...
test('empty authored asset set plans deletion of a remote set', () => {
  ...
  const plan = createStoreListingPlan({ desired: emptyDesired, currentRevision: 'current', states: [current] });
  expect(plan.steps.map((step) => step.operation)).toEqual(['replace-assets']);
});

test('error diagnostics block mutation planning', () => {
  const blocked = { ...current, diagnostics: [{ severity: 'error', code: 'X', message: 'blocked' }] } as const;
  expect(createStoreListingPlan({ desired, currentRevision: 'current', states: [blocked] }).status).toBe('blocked');
});
